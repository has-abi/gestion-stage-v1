...
public class OrganismeRest {
	...
	@GetMapping("/search")
	public ResponseEntity<List<OrganismeAccueil>> searchForOrganismes(
			@SearchSpec Specification<OrganismeAccueil> spec) {
		return organismeAccueilService.searchForOrganismes(spec);
	}

	@GetMapping("/type/{type}/page/{page}/size/{size}")
	public Page<OrganismeAccueil> findByTypeOrganismeType(@PathVariable String type, @PathVariable int page,
			@PathVariable int size) {
		return organismeAccueilService.findByTypeOrganismeType(type, page, size);
	}

	@GetMapping("/typeService/{type}/page/{page}/size/{size}")
	public Page<OrganismeAccueil> findByTypeServiceOrganismeType(@PathVariable String type, @PathVariable int page,
			@PathVariable int size) {
		return organismeAccueilService.findByTypeServiceOrganismeType(type, page, size);
	}

	@GetMapping("/ville/{nom}/page/{page}/size/{size}")
	public Page<OrganismeAccueil> findByVilleNom(@PathVariable String nom, @PathVariable int page,
			@PathVariable int size) {
